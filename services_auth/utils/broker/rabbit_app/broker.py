...
async def lifespan(app: FastAPI):
    ...
    while not isconnect or pingcounter < 15:
        try:
            await rabbit_broker.broker.connect(settings.RABBITMQ_URL)
            isconnect = True
...
rabbit_broker = RabbitRouter(schema_url="/asyncapi", include_in_schema=True, middlewares=[LoggerMiddleware], logger=logger, lifespan=lifespan)