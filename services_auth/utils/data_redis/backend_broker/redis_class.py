...
from schemas.services_second_schemas import ABCSchemaGet, Status
...
class RedisManeger(RedisABC, Singleton):
    ...
    async def get_data(self, key: str, execution_model: pydantic.BaseModel, timeout: float=0.1):
        ...
        assert resul.status == Status.success, (f'message: {resul.details.model_dump()}' f'status_code: {resul.status_code}')
